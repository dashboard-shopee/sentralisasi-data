          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-t border-[#f3f4f8] hover:bg-[#fafbfd]">
                ...
                {columns.map((c) => {
                  if (c.edit) {
                    const val = edits[String(r.kode)]?.[c.key] ?? "";
                    return (
                      <td key={c.key} className="px-2 py-1.5">
                        <input
                          ...
                          placeholder="—"
                          className="w-[100px] border border-[#e6e9f0] rounded-md px-2 py-1 text-right text-[12px] focus:border-[#ee4d2d] outline-none"
                        />
                      </td>
                    );
                  }
                  if (c.computeMax) {
                    ...
                    return (
                      <td key={c.key} className="px-3 py-2 whitespace-nowrap text-right tabular-nums font-semibold">
