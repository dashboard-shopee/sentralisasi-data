...
- **M1 Fase 1 lengkap** ✅ (14 Jul) — Garansi 2-kolom (`floor_price`=Terbaik + `ceiling_price`=Program dari `bidding_info`, merge authoritative dari `list_ongoing_status` by bid_id; `baca_garansi_best` expose terbaik+program) · Voucher `fe_status` (kebenaran status hidup 1/2/3) · `_buang_berakhir` — AUDIT: semua 6 modul udah drop BERAKHIR (paket `_buang_berakhir`, flash `hanya_aktif`, voucher pt2/1, promo_toko time_status∈1,2, campaign window, garansi ongoing) · **produk stok-habis→stok=0** (`nolkan_stok_habis` timestamp-based abis grab LENGKAP — AKAR voucher poison KELAR) · auto-isi harga diskon Fase 1 ✅ (`isi_harga_diskon_kosong` udah jalan tiap grab; Fase 3 nempel pas M2). Migrasi additif applied+verified di DB.
- **M2 Loop A→B + Fase 3 + alasan** ✅ (14 Jul) — `siklus_terpadu` jadi 2 loop: LOOP A (fase1+2 semua toko, simpen `keputusan`) → LOOP B (Fase 3 laporan: grab-ulang SEMUA modul `_grab_fakta` + `alasan_terkini`). Jeda propagasi gratis dari lamanya Loop A. `alasan` per produk ke `harga_olah_data.alasan` (infra `tulis_alasan` existing) = narasi aksi Loop A (`bangun_alasan`) + verifikasi terkini (`✓ sesuai`/`⚠ belum (real X)`). Heartbeat `catat_fase('laporan')` + pemicu dashboard. Nyala kalau `FASE_AKTIF ⊇ {3}`. BELUM live (butuh grab hidup — verif di M4).
- **M3 Poin 1–4 harga + REM** ✅ (14 Jul, logika — BELUM live) 🔴 — **rem 30%/40%** (`kena_rem_toko` gate eksekusi >30% produk keubah; `rem_turun` per-produk target<60% Harga Diskon, `baca_baris_rubah` +harga_diskon mentah) · **komisi peg +trigger Shopee** (`baca_komisi_shopee_aktif`; Shopee-aktif tanpa harga_jual Syntra → `komisi_hold` jaga harga) · **garansi takedown jam 2-kolom** (undercut @Terbaik/floor · margin @Program/ceiling per grilling) · **poin 4 re-attach→provisioning harian** (buang re-add inline; ubah harga awal = band voucher geser → provisioning reconcile band bener). ⚠️ Tes scope 1 toko + DRY dulu sebelum LIVE.
- **M4 Poin 5 provisioning** 🔧 (15 Jul) — garansi/voucher/paket ✅ verified live (1171/1174 sesuai, voucher fe_status=2). Flash ✅: bug per-model ke-fix (harga model murah ke-max-in ke model mahal) + self-heal real-time (sniff: `set_shop_flash_sale` status 0=hapus/1=aktif/2=stop, alur stop→hapus→bikin sesi baru di slot sama→daftar ulang produk sehat, terverifikasi slot beneran reusable). Campaign 🔧 root cause = endpoint `get_nominated`/`opt_out` WAJIB browser-context (bukan requests polos); grab+takedown+provisioning SEMUA dipindah ke `campaign_util.py`, scope `CAMPAIGN_KEYWORDS` (tanggal kembar) — belum dites live.
