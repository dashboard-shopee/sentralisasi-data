...
✅ **SOLVED 15 Jul (tahap 1)** — sniff owner manual set/takedown 1 produk nemuin: `nominated_entity_list` cuma lolos kalau browser NAVIGASI BENERAN ke halaman detail sesi (page render Shopee sendiri yg manggil, otomatis lolos signature). Tapi ini CUMA jalan kalau sesi itu default-tab-nya "Dinominasikan" pas dibuka (sesi bersih tanpa draft nyangkut) — kebukti live sesi 957815 (12/12 kebaca). Sesi yg ada draft nyangkut (default-tab geser ke "Menunggu Didaftarkan") GAK kebaca (0 hasil, false-negative) — dan MAKSA klik tab manual via DrissionPage kena **`verify/traffic/error` (captcha WAF Shopee)**, bukan cuma 90309999 — jalan buntu total, GA BISA di-otomasi.
5. **fill_recommend_price bug** — `preview/add` tanpa `fill_recommend_price:true` + `operate_start_time` bikin `campaign_price` selalu 0 → submit GAGAL 100% ("Harga promo harus > 0"). Ketauan dari screenshot owner (harga Rp0 di form UI). **FIXED** — Shopee sendiri yg ngitung harga rekomendasi per model.

✅ **SOLVED 15 Jul (FINAL, "full otomatis")** — nomination_id GAK PERLU dibaca dari `nominated_entity_list` sama sekali. Ternyata nomination_id UDAH kebentuk begitu produk di-STAGE (`preview/add`), dan endpoint `preview_list` (beda dari `nominated_entity_list`) OTOMATIS fire pas navigasi biasa ke halaman sesi — TANPA klik, TANPA captcha. Jadi: `campaign_util.nominate()` sekarang, abis staging (sebelum submit), navigasi+dengerin `preview_list` (`get_preview_nomination_ids()`) buat nangkep nomination_id tiap model, lalu SIMPEN SENDIRI ke DB (`harga_fakta_campaign_item`, `SQL.upsert_fakta_campaign_item` — upsert incremental, BUKAN snapshot/delete-all). `takedown_campaign.py` & `provisioning.campaign()` sekarang baca nomination_id/status "udah ternominasi" dari **DB kita sendiri** (`SQL.baca_campaign_item`), bukan query live ke Shopee — jadi nominate+takedown 100% otomatis, gak nyentuh endpoint yg diblokir/kena captcha sama sekali.
Verified live 16 Jul (kimmioshop, sesi 978125): nominate 3 model baru → DB kecatet 4 baris (3 baru + 1 draft lama nyangkut) dgn nomination_id lengkap → `get_session_nomination_statistics` konfirmasi count naik → takedown 3 nomination_id dari DB → count turun balik. `get_nominated_products()` (navigasi-listen versi lama) DIPERTAHANKAN sbg pembacaan live best-effort buat dashboard (`fakta_campaign`) — jalan buat sesi bersih, tapi BUKAN lagi sumber kebenaran buat nominate/takedown.
