...
- [~] **S4. Task 7 (rem paket) + Task 8 (bisect voucher) live-test.** Probe 17 Jul ZIO+BEVERRA: `bundle_deal/list` **error keras 1400101507** → `_call` raise → toko skip (rem LAMA, aman ga dobel). Rem BARU (list KOSONG+code 0+total_count>0) belum ketemu kejadian nyata — logic mock-verified, kepasang nunggu. Task 8 bisect: butuh live create di toko poison → ACC owner.
